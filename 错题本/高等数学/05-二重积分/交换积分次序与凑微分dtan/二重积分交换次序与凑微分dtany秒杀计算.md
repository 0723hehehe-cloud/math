...
type: 错题
tags:
  - 错题本/高等数学/05-二重积分/交换积分次序与凑微分dtan
...
> 顺利交换次序算完内层积出 $(\\tan y)^{{1/3}}$ 后，面对外层被积函数 $\\\csc 2y \\\cdot (\\tan y)^{{1/3}}$ 感到棘手卡住，没有想到利用三角恒等式 $\\\csc 2y = \\frac{{\\sec^2 y}}{{2\\tan y}}$ 凑微分出 $d(\\tan y)$！

> [!tip] 💡 核心秒杀法宝：【交换次序 + 凑微分 $d(\\tan y)$】
> 1. **交换积分次序**：原区域 $0 \\le x \\le 1, \\arctan x \\le y \\le \\frac{{\\pi}}{{4}}$ 转化为先 $x$ 后 $y$：
