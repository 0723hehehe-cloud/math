...
type: 解题技巧与易错陷阱
tags:
  - 解题技巧与易错陷阱/高等数学/05-二重积分/极坐标换限与反三角函数定义域/极坐标积分范围与arcsin(sin x)主值定义域易错陷阱
---
...
>
> 3. **精准处理反三角主值与外层积分**：
>    因 $\theta n [\frac{3\pi}{4}, \pi]$，则 $\arcsin(\sin \theta) = \arcsin(\sin(\pi-\theta)) = \pi - \theta$。
>    代入并分步计算得出最终结果：
>    $$I = \frac{\pi}{4} \ln(\sqrt{2}+1) - \frac{\pi^2}{32}$$

> [!warning] 易错点 & 核心考点
> ⚠️ **避坑死穴**：
> 1. **反三角脱帽规则**：$\arcsin(\sin x) = x$ **仅在 $x n [-\frac{\pi}{2}, \frac{\pi}{2}]$ 时成立**！若在第二象限必写为 $\pi - x$。
> 2. **极坐标半径正负校验**：极坐标半径 $r$ 必须大于零，当 $\theta n [\frac{3\pi}{4}, \pi]$ 时 $\cos \theta \le 0$，故 $r = -\sec \theta > 0$。