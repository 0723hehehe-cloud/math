...
type: 错题
tags:
  - 解题技巧与易错陷阱/高等数学/05-二重积分/极坐标换限与反三角函数定义域
...
> 在计算内层积分 $\int \frac{dr}{\sqrt{4-r^2}} = \arcsin\frac{r}{2}$ 并代入下限 $r = 2\sin \theta$ 时，把 $\arcsin(\sin \theta)$ 错写成了 $\theta$。
> 忽视了当 $\theta \in [\frac{3\pi}{4}, \pi]$ 时，$\theta$ 已经超出了 $\arcsin$ 的主值范围 $[-\frac{\pi}{2}, \frac{\pi}{2}]$，正确化简应当是 $\arcsin(\sin \theta) = \pi - \theta$！

> [!success] 解析与答案
> 1. **画图与极坐标换限**：
>    - 区域 $D$ 在第二象限，角度范围为 $\frac{3\pi}{4} \le \theta \le \pi$。
>    - 下边界 $y = 1 - \sqrt{1-x^2} \iff x^2 + (y-1)^2 = 1 \implies r = 2\sin \theta$。
